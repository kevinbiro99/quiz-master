<template>
  <div>
    ...
    <div v-if="!questionEnded">
      ...
      <div v-for="(choice, index) in currentQuestion.choices" :key="index" @click="selectAnswer(index)" :class="{ selected: selectedAnswer === index }">
        {{ choice }}
      </div>
    ...
    <div v-else>
      ...
      <div v-for="(choice, index) in currentQuestion.choices" :key="index" :class="{ selected: selectedAnswer === index, correct: index === optionsMap[correctAnswer] }">
...
export default {
  ...
  methods: {
    ...
    loadNextQuestion() {
      ...
      this.currentQuestion.choices = [question.option1, question.option2, question.option3, question.option4];
    ...
    selectAnswer(index) {
      ...
      const score = (this.selectedAnswer === this.optionsMap[this.correctAnswer] ? 1 : 0) * (1 + this.answerTimeLeft);
